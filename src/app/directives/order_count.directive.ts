...
export class OrderCountDirective {
    ...
    set order(order: Order) {
        if (order.order_total_count >= 0 && order.attendance.status == AppConstants.WORKING) {
        ...
        if (order.attendance.status) {
            if (order.attendance.status != AppConstants.WORKING)
                this.el.nativeElement.innerText = order.attendance.status.charAt(0).toUpperCase();

            // set background color depending on status
            if (order.attendance.status == AppConstants.LEAVE)
                this.el.nativeElement.style.backgroundColor = this.leave;
            else if (order.attendance.status == AppConstants.HOLIDAY)
                this.el.nativeElement.style.backgroundColor = this.holiday;
        }
    }
}
