...
export class AttendancePobStatusDirective {
     ...
     */
    formatCell() {
        if (this._att.pob_amount >= 0 && this._att.status == AppConstants.WORKING) {

            // set text value
            this.el.nativeElement.innerText = (this._att.pob_amount / 1000).toFixed(1);

            // set background color depending on status
            if ((this._att.pob_amount/1000) < this._target)
