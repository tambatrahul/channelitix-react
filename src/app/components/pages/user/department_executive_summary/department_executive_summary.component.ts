...
@Component({
  templateUrl: 'department_executive_summary.component.html',
  styleUrls: ['department_executive_summary.component.less']
})
export class DepartmentExecutiveSummaryComponent extends ListComponent {
        ...
        area.headquarters.map(headquarter => {
          primarySalesAndTargets.map(target => {
            if (target.hq_headquarter_id == headquarter.id) {
              ...
              console.log(target.sub_name);
              if (target.sub_name == 'Encube') {
                headquarter.encube_target += target.total_target ? target.total_target : 0;
                headquarter.encube_primary += target.total_net_amount;
              }

              if (target.sub_name == 'Accuchek') {
                headquarter.accuchek_target += target.total_target ? target.total_target : 0;
                headquarter.accuchek_primary += target.total_net_amount;
              }
              if (target.sub_name == 'Accufine') {
                headquarter.accufine_target += target.total_target ? target.total_target : 0;
                headquarter.accufine_primary += target.total_net_amount;
              }
              ...
              if (target.hq_headquarter_id == headquarter.parent_headquarter_id) {
                ...
                if (target.sub_name == 'Encube') {
                  area.encube_target += target.total_target ? target.total_target : 0;
                  area.encube_primary += target.total_net_amount;

                  region.encube_target += target.total_target ? target.total_target : 0;
                  region.encube_primary += target.total_net_amount;
                }

                if (target.sub_name == 'Accuchek') {
                  area.accuchek_target += target.total_target ? target.total_target : 0;
                  area.accuchek_primary += target.total_net_amount;

                  region.accuchek_target += target.total_target ? target.total_target : 0;
                  region.accuchek_primary += target.total_net_amount;
                }

                if (target.sub_name == 'Accufine') {
                  area.accufine_target += target.total_target ? target.total_target : 0;
                  area.accufine_primary += target.total_net_amount;

                  region.accufine_target += target.total_target ? target.total_target : 0;
                  region.accufine_primary += target.total_net_amount;
                }
      ...
      region.areas.map(area => {
        area.headquarters.map(headquarter => {
          orders.map(ord => {
            ...
            if (ord.hq_headquarter_id == headquarter.id) {
              if (ord.sub_name == 'Encube') {
                headquarter.encube_total_pob += ord.order_total_count;
                area.encube_total_pob += ord.order_total_count;
                region.encube_total_pob += ord.order_total_count;
              }
            }

            if (ord.hq_headquarter_id == headquarter.id) {
              if (ord.sub_name == 'Accuchek') {
                headquarter.accuchek_total_pob += ord.order_total_count;
                area.accuchek_total_pob += ord.order_total_count;
                region.accuchek_total_pob += ord.order_total_count;
              }
            }

            if (ord.hq_headquarter_id == headquarter.id) {
              if (ord.sub_name == 'Accufine') {
                headquarter.accufine_total_pob += ord.order_total_count;
                area.accufine_total_pob += ord.order_total_count;
                region.accufine_total_pob += ord.order_total_count;
              }
            }
