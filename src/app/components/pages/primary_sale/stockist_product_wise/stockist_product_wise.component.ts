...
export class StockistProductWiseComponent extends ListComponent {
  ...
  public total: number = 10;
    ...
    set month(month: number) {
        this._month = month;
        this.fetchStockistProductWiseSale();
    }
   ...
   * fetch from server
   */
  fetchStockistProductWiseSale = AppConstants.debounce(function () {
    let self = this;
    if ((self._month || self._month == 0) && self._year && !this.loading) {
      ...
      this.loading = true;
      self.saleService.monthly_stockist_product(self._month + 1,
        ...
        response => {
          ...
          self.loading = false;
          setTimeout(() => {
            self.upload_excel = jQuery('#stockist_product_wise_sale1').tableExport({
              formats: ['xlsx'],
              bootstrap: true,
              position: 'top'
            });
  ...
  download() {
    ...
    this.saleService.stockist_product_excel_download(this._month + 1, this._year,
      ...
      response => {
        ...
        link.download = 'primary_sales_stockist_product_report.xls';
        link.click();
